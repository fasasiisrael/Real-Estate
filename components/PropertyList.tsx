...
const PropertyList: React.FC = () => {
  ...
  const [dateRange, setDateRange] = useState({ min: '2000-03-25', max: '2024-09-25' });
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      try {
        const response = await axios.post('https://realty-in-us.p.rapidapi.com/properties/v3/list', {
          limit: 200,
          offset: 0,
          boundary: {
            coordinates: [
        ...
        if (response.data && response.data.data && response.data.data.home_search && response.data.data.home_search.results) {
  ...
  useEffect(() => {
    ...
    const applyFilters = () => {
      const result = properties.filter((property: any) => {
        const isInPriceRange = property.list_price >= priceRange.min && property.list_price <= priceRange.max;
        const isInDateRange = new Date(property.list_date) >= new Date(dateRange.min) && new Date(property.list_date) <= new Date(dateRange.max);
        const matchesSearch = property.description.type.toLowerCase().includes(search.toLowerCase()) ||
                              property.location.address.line.toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <div className="relative p-4">
      <div
        ...
        style={{
          backgroundImage: 'url(https://thumbs.dreamstime.com/b/real-estate-vector-thin-line-seamless-pattern-business-122886196.jpg)',
      ...
      <div className="relative z-10 flex items-center">
        <input
          ...
          onBlur={(e) => e.target.placeholder = 'Have a specific house or place in mind?'}
        />
        <button
        ...
        >
          ...
          <img width="25" height="25" src="https://img.icons8.com/sf-black/25/FFFFFF/sorting-options.png" alt="sorting-options"/>
        </button>
      </div>
      ...
      {loading && (
        <div className="flex justify-center items-center h-64">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" width="100" height="100">
            <circle fill="#84DFC4" stroke="#84DFC4" strokeWidth="2" r="15" cx="40" cy="65">
            ...
            <circle fill="#84DFC4" stroke="#84DFC4" strokeWidth="2" r="15" cx="100" cy="65">
            ...
            <circle fill="#84DFC4" stroke="#84DFC4" strokeWidth="2" r="15" cx="160" cy="65">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {currentProperties.map((property: any) => {
          ...
          return (
            <Link href={`/property/${property.property_id}`} key={property.property_id} legacyBehavior>
              <a className="relative block">
                <PropertyCard
                  image={property.primary_photo?.href || 'https://web.redanonline.org/wp-content/uploads/2021/12/real-estate-business-compressor.jpg'}
                  name={property.description.type || 'Unknown'}
                  location={property.location.address.line || 'Unknown'}
                  price={property.list_price || 0}
                  listingDate={listingDate}
                  contact={property.advertisers && property.advertisers.length > 0 ? property.advertisers[0].email : 'N/A'}
